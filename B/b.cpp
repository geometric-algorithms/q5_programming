...
pair<Point, Point> normalize(const Point& a, const Point& b) {
    return a < b ? make_pair(a, b) : make_pair(b, a);
}

set<pair<Point, Point>> existingSegments;
...
int main() {
    int n;
    cout << "Enter initial number of segments (n), followed by n segment coordinates (x1 y1 x2 y2): " << endl;
    cin >> n;
    
    for (int i = 0; i < n; ++i) {
        Point p, q;
        cin >> p.x >> p.y >> q.x >> q.y;
        auto key = normalize(p, q);
        
        if (existingSegments.insert(key).second) {
            segments.emplace_back(p, q, nextId++);
        }
    }
    cout << "Type 0 to find intersections, 1 to add segment, 2 to remove segment, 3 to exit" << endl;
    cout << "Enter 0, 1, or 2 followed by segment coordinates (x1 y1 x2 y2):" << endl;
    int type;
    while (cin >> type) {
        if (type == 0) {
            ...
            if (type == 1) {
                if (existingSegments.count(key)) {
                    cout << "Segment already present, choose different segment" << endl;
                } else {
                    cout << "Adding segment" << endl;
                    cout << "Finding intersections..." << endl;
                    existingSegments.insert(key);
                    segments.emplace_back(p, q, nextId++);
                }
            } 
            else if (type == 2) {
                auto it = existingSegments.find(key);
                if (it == existingSegments.end()) {
                    cout << "Segment not present, choose existing segment" << endl;
                } else {
                    cout << "Removing segment" << endl;
                    cout << "Finding intersections..." << endl;
                    existingSegments.erase(it);
                    // Remove from vector
                    segments.erase(
                        remove_if(segments.begin(), segments.end(),
                            [&](const Segment& s) {
                                return normalize(s.p, s.q) == key;
                            }),
                        segments.end()
                    );
                }
